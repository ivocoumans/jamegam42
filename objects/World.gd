...
func _generate_platform(x_min, x_max):
	...
	_render_platform(Rect2(x, y, width, height))
...
func _generate_hole(x_min, x_max):
	...
	_render_hole(Rect2(x, y, width, height))
...
func _generate_pillar(x_min, x_max):
	...
	_render_pillar(Rect2(x, y, width, height))
	
	return Feature.new(x, y, width, height, 2)


func _find_feature_in_vicinity(x, _y, width, _height):
	var rect = Rect2(x - FEATURE_MARGIN, PLATFORM_Y_BEGIN, width + FEATURE_MARGIN, GROUND_Y_END)
	for feature in features:
		if rect.intersects(feature.rect):
			return true
	return false
...
func _find_prop_in_vicinity(x, y, width, height):
	var rect = Rect2(x - PROP_MARGIN, y - PROP_MARGIN, width + PROP_MARGIN, height + PROP_MARGIN)
	for prop in props:
		if rect.intersects(prop.rect):
			return Vector2(x, y)
	return null
